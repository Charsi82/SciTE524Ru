...
local grbox1 = gui.groupbox("GroupBox")
tab4:add(grbox1,"bottom",60)
grbox1:position(20,115)
grbox1:size(250, 60)

tab4:on_command(function(id)
...
tab4:on_scroll( function(id, pos) label1:set_text("scroll pos: " .. pos) end)
--[[local tree = gui.tree() -- haslines[true], editable[false]
tab4:client(tree)
local itm1 = tree:add_item("item1") -- ("caption", parent_item [or root])
tree:add_item("item2", itm1)
tree:add_item("item3")
local itm4 = tree:add_item("item4")
local itm5 = tree:add_item("item5", itm4)
local itm6 = tree:add_item("item6", itm5)
local itm7 = tree:add_item("item7", itm6)]]

-- tree:set_selected_item(itm5)
-- treeview:tree_item_get_text( itm )
-- print("test:",tree:tree_item_get_text(itm5))

-- AddEventHandler("OnInit", function()
-- tree:on_select(function(itm) print("tree: select ",tree:tree_item_get_text(itm)) end)
-- tree:on_double_click(function(imt) print("tree: db click on ",tree:tree_item_get_text(itm)) end)
--  end, 'RunOnce')

-- TODO  get selected item

-- tree:on_key(function(...) print("tree: onkey",...) end)
-- function treee_test() print('tree test') end --ok
-- tree:context_menu{"item1|treee_test"} -- ok
-------------------------
local tabs = gui.tabbar(win_parent)
tabs:add_tab("Проводник", tab0)
tabs:add_tab("Функции", tab1)
tabs:add_tab("Сокращения", tab2)
tabs:add_tab("ASCII", tab3)
tabs:add_tab("Test", tab4)
