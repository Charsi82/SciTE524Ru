...
const char* CallNamedFunction(const char* name, unsigned int numberArg, const char* stringArg, lua_Integer numberArg2) {
	const char* handled = NULL;
	if (luaState) {
		lua_getglobal(luaState, name);
		if (lua_isfunction(luaState, -1)) {
...
const char* CallNamedFunction(const char* name, unsigned int numberArg, unsigned int numberArg2, lua_Integer numberArg3) {
	const char* handled = NULL;
	if (luaState) {
		lua_getglobal(luaState, name);
		if (lua_isfunction(luaState, -1)) {
...
std::string CallNamedFunction(const char* name, unsigned int numberArg) {
	std::string handled;
	if (luaState) {
		lua_getglobal(luaState, name);
		if (lua_isfunction(luaState, -1)) {
