		...
		if (style != SCE_ERR_DEFAULT &&
			...
			if (sourceLine >= 0) {
				...
				if (props.GetInt("lexer.errorlist.findliststyle", 1) && '.' == source[0] && pathSepChar == source[1]) {
					...
					SA::Line topLine = lookLine - 1;
					std::string topic;
					while (topLine >= 0) {
						SA::Position startPos = wOutput.PositionFromLine(topLine); //wOutput.Call(SCI_POSITIONFROMLINE, topLine, 0);
						SA::Position lineLen = wOutput.LineLength(topLine); //wOutput.Call(SCI_LINELENGTH, topLine, 0);
						topic = wOutput.StringOfSpan(SA::Span(startPos, startPos + lineLen)); //GetRangeString(wOutput, startPos, startPos + lineLen);
						if ('>' == topic[0]) {
							break;
						}
						else {
