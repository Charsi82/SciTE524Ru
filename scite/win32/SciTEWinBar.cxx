...
//!-end-[close_on_dbl_clk]
	...
	case WM_PAINT: {
			...
			if ( toptab_h ) {
				...
				HDC hDC = ::GetDC(hWnd);
				if (hDC) {
					tabrc.bottom = tabrc.top + toptab_h; // height of topbar
...
void SciTEWin::SetToolBar() {
	...
	GUI::gui_string sIconlib = GUI::StringFromUTF8(props.GetNewExpandString("user.toolbar.iconlib.", fileNameForExtension.c_str()));
	if (sIconlib.empty()) sIconlib = GUI::StringFromUTF8(props.Expand("$(SciteDefaultHome)\\toolbar\\cool.dll"));
	HICON hIcon = NULL;
	HICON hIconBig = NULL;
	long iIconsCount = 0;
	//TArray<HICON, HICON> arrIcons;
	std::vector<HICON> arrIcons;
	while ((int)::ExtractIconExW(sIconlib.c_str(), iIconsCount++, &hIconBig, &hIcon, 1) > 0) {
		...
		if (hIcon != NULL) arrIcons.push_back(hIcon);
	}

	HBITMAP hToolbarBitmapNew = 0;
	//iIconsCount = arrIcons.GetSize();
	iIconsCount = arrIcons.size();
	if (iIconsCount > 0) {
		SIZE szIcon = { 16, 16 };
