...
static void lua_pushlasterr(lua_State* L, const wchar_t* lpszFunction)
{
	...
	{
		lua_pushstring(L, UTF8FromString(lpMsgBuf).data());
	}
	else
	{
		UINT uBytes = (iLenMsg + lstrlen(lpszFunction) + 40) * sizeof(char);
		//wchar_t* lpDisplayBuf = (wchar_t*)::LocalAlloc( LMEM_ZEROINIT, uBytes );
		//swprintf_s( lpDisplayBuf, uBytes, L"%s failed with error %d: %s", lpszFunction, dw, lpMsgBuf );
		//lua_pushstring( L, UTF8FromString(lpDisplayBuf) );
		//::LocalFree( lpDisplayBuf );
		wchar_t* lpDisplayBuf = new wchar_t[uBytes] {0}; //(LMEM_ZEROINIT, uBytes);
		swprintf_s(lpDisplayBuf, uBytes, L"%s failed with error %lu: %s", lpszFunction, dw, lpMsgBuf);
		lua_pushstring(L, UTF8FromString(lpDisplayBuf).data());
		delete[]lpDisplayBuf;
	}
	::LocalFree(lpMsgBuf);
}
...
static BOOL RunProcessHide(CPath& path, DWORD* out_exitcode, CSimpleString* strOut)
{
	...
	STARTUPINFOW si = {  }; si.cb = sizeof(STARTUPINFOW);
...
static int getclipboardtext(lua_State* L)
{
	...
	lua_pushstring(L, UTF8FromString(clipText.GetString()).data());
	return 1;
}
